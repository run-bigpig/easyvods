function show_history(cookie_name) {
    var history = $.cookie("ev_history_dianying");
    var history_data = [];
    var history_html = '';
    ...
    if (history_data.length > 0) {
        for (var $i = 0; $i < history_data.length; $i++) {
            history_html += '<li class="list-item"><a href="' + history_data[$i].vod_url + '" title="' + history_data[$i].vod_name + '" class="list-item-link"><i class="icon-play"></i>' + history_data[$i].vod_name + '</a></li>';
        }
    } else {
...
$(function () {
    ...
    $(".historyclean").on("click", function () {
        $.cookie("ev_history_dianying", null, {expires: -1, path: '/'});
    });
    ...
    $(".sourcesearch").click(function () {
        let kw = $('#txtKeywords').val()
        let sourceid = $(this).attr("source")
        if (kw == "" || sourceid == "") {
            alert("请先输入想看得视频名称")
            return false;
        }
        location.href = "/search?sourceid=" + sourceid + "&cz=" + kw
    })
});
